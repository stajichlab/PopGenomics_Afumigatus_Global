...
for (window in windows) {
  ...
  f <- ggplot(repeatwindows, aes(x = pos, y = NormDepth, color = Strain)) +
    ...
    scale_y_continuous(name = "Normalized Read Depth", limits = c(0, 3)) +
    theme_classic()
  # break these up
  ...
  strainID = unique(d$Strain)
  plts <- lapply(strainID, plot_strain, data = d, repeats=rd, genes=rg)
  
  for (i in 1:length(strainID)) {
